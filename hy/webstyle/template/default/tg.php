...
$query=$db->query("SELECT SQL_CALC_FOUND_ROWS * FROM {$pre}tuangou_content $where ORDER BY posttime DESC LIMIT $min,$rows");
$RS=$db->get_one("SELECT FOUND_ROWS()");
$totalNum=$RS['FOUND_ROWS()'];
$showlists='';
while($rs=$db->fetch_array($query)){
	$rs[picurl]=$rs[picurl]?tempdir($rs[picurl]):"$webdb[www_url]/images/default/nopic.jpg";
	$moredb=$db->get_one("SELECT * FROM {$pre}tuangou_content_$rs[mid]  WHERE id='$rs[id]'" );
	$rs[jointime]=$moredb[jointime]?$moredb[jointime]:'不限';
	$rs[end_time]=$moredb[end_time]?$moredb[end_time]:'不限';
	$showlists.="<dl>
					<dt><a href='/tg/bencandy.php?fid=$rs[fid]&id=$rs[id]' target='_blank'><span><img src='$rs[picurl]'></span></a></dt>
					<dd>
						<ul>
							<ol><a href='/tg/bencandy.php?fid=$rs[fid]&id=$rs[id]' target='_blank'>$rs[title]</a></ol>
							<li><span style='color:#666'>开始时间:</span>{$rs[jointime]}</li>
						</ul>
					</dd>
				</dl>";
}
...
//-->
	...
	<div class="RightContainers">
		<div class="RightMainBox">
			<div class="head"><span class="tag">商家活动<em>&nbsp;</em></span></div>
			<div class="cont">
				<div class="ListShops">
$showlists
				</div>				
