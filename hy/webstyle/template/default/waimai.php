...
$min=($page-1)*$rows;
$where=" WHERE uid='$uid'";
$query=$db->query("SELECT SQL_CALC_FOUND_ROWS * FROM {$pre}waimai_company $where ORDER BY posttime DESC LIMIT $min,$rows");
...
while($rs=$db->fetch_array($query)){
	...
	$showlists.="<dl>
					<dt><a href='/waimai/bencandy.php?fid=$rs[fid]&id=$rs[id]' target='_blank'><span><img src='$rs[picurl]'></span></a></dt>
					<dd>
						<ul>
							<ol><a href='/waimai/bencandy.php?fid=$rs[fid]&id=$rs[id]' target='_blank'>$rs[title]</a></ol>
							<li><span style='color:#888;'>起送价：</span>&yen;{$rs[price]}</li>
						</ul>
					</dd>
				</dl>";
}
...
//-->
	...
	<div class="RightContainers">
		<div class="RightMainBox">
			<div class="head"><span class="tag">外卖店铺<em>&nbsp;</em></span></div>
			<div class="cont">
				<div class="ListShops">
$showlists
				</div>				
