...
$linkdb=array(
			...
			"未认证的企业"=>"$admin_path&job=list&renzheng=0&pictype=$pictype",
			"普通认证的企业"=>"$admin_path&job=list&renzheng=1&pictype=$pictype",
			"银牌认证的企业"=>"$admin_path&job=list&renzheng=2&pictype=$pictype",
			"金牌认证的企业"=>"$admin_path&job=list&renzheng=3&pictype=$pictype",
			);
...
if($job=="list")
{
	...
	if(in_array($pictype,array('permit_pic','guo_tax_pic','di_tax_pic','organization_pic','idcard_pic'))){
		$SQL.=" AND `$pictype`!='' ";
	}
