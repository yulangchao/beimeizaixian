...
//公司资料修改
if($job=='showsort'){
	$show="<option value='0'>请选择栏目</option>\r\n";
...
if($job=='selsort'){
	$fup=$Fid_db[fup][$fid];
	$show="<div><input type='checkbox' name='fids[]' value='$fid' checked> {$Fid_db[name][$fup]} &gt {$Fid_db[name][$fid]}</div>";
	echo $show;
	exit;
}
...
if($act=='editlogo'){
	...
	if($picurl){
		...
		delete_attachment($uid,tempdir($intro[picurl]));
		$db->query("UPDATE `{$_pre}company` SET `picurl`='$picurl' WHERE uid='$uid'");
	}
	die(tempdir($picurl));
}
...
if(!$step){
	...
	$qy_pro_ser=filtrate($_POST[qy_pro_ser]);
	$my_buy=filtrate($_POST[my_buy]);
	$qy_regplace=filtrate($_POST[qy_regplace]);

	//`picurl`='$picurl',商家logo改为异步修改
