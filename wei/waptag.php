...
$listdb='';
$query = $db->query("SELECT SQL_CALC_FOUND_ROWS B.* FROM {$_pre}keywordid A LEFT JOIN {$_pre}content B ON A.id=B.id WHERE A.kid='$ks[kid]' $SQL ORDER BY B.id DESC LIMIT $min,$rows");
...
$showpage=getpage("","","waptag.php?keyword=".urlencode($keyword),$rows,$totalNum);
