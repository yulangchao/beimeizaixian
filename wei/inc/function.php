...
function list_hot_keyword($rows=100){
	global $db,$_pre;
	$rows=intval($rows);
	$query =$db->query("SELECT * FROM {$_pre}keyword ORDER BY num DESC LIMIT $rows");
	while($rs =$db->fetch_array($query)){
		if($rs[color]!=''){
			$rs[style] = "color:$rs[color];";
		}
		if($rs[bold]){
			$rs[style] .= "font-weight:bold;";
		}
		$listdb[]=$rs;
	}
	return $listdb;
...
function get_txt_word($content,$leng=0){
	$content = preg_replace('/<([^<]*)>/is',"",$content);
	if($leng>0){
		$content = get_word($content,$leng);
	}
	return $content;
}
...
function show_keyword($content){
	...
	require_once(ROOT_PATH."data{$GLOBALS[webdb][web_dir]}/keyword.php");
	//把图片描述去掉
