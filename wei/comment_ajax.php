...
while( $rs=$db->fetch_array($query) )
{
	...
	if($web_admin){
		$delword="<a href='javascript:;delcomments($rs[id],$rs[cid])' class='del'>删除</a>";
	}
	$show.="<div class='comments'>
			<div>
				<a href=\"$webdb[www_url]/member/homepage.php?uid=$rs[uid]\">$rs[username]</a> <span>$rs[posttime]</span>
				$delword
			</div>
			<p>$rs[content]</p>
		</div>";
...
$showpage=preg_replace("/\?fid=([\d]+)&id=([\d]+)&page=([\d]+)/is","javascript:getcomments('comment_ajax.php?fid=\\1&id=\\2&page=\\3')",$showpage);

$showpage&&$show.="<div class='commentpage'>$showpage</div>";
