...
//获取标签内容
if($fidDB[type]==2){
	$template_file=getTpl("wapalonepage");
}elseif($fidDB[type]==1){
	$template_file=getTpl("wapbigsort",$main_tpl);
}else{
...
{	
	...
	$showpage=getpage("{$_pre}content A",$SQL,"waplist.php?fid=$fid",$Lrows);	
}
...
if($job=="showmore"){
	if(!$listdb){
		...
		foreach($listdb AS $key=>$rs){
			...
			echo "<li><a href=\"$rs[url]\">$rs[title]</a></li>";
		}
	}
	exit;
}
