...
@include(dirname(__FILE__)."/"."data/wapguide_fid.php");
...
if(!$jobs && $rsdb[city_id]!=$city_id){
	if($city_DB[domain][$rsdb[city_id]]){
		echo "<META HTTP-EQUIV=REFRESH CONTENT='0;URL={$city_DB[domain][$rsdb[city_id]]}/{$ModuleDB[$webdb[module_pre]][dirname]}/wapbencandy.php?fid=$fid&id=$id'>";
		exit;
	}
}
...
$showpage=getpage("","","wapbencandy.php?fid=$fid&id=$id",1,$rsdb[pages]);

if($rsdb[iframeurl])
{
	$head_tpl=$foot_tpl="template/default/wapnone.htm";
	$template_file="template/default/wapbencandy_iframe.htm";
}
...
$showpage = $rsdb[pages]>1?getpage('','',"wapbencandy.php?fid=$fid&id=$id",1,$rsdb[pages]):'';
...
function show_keyword($content){
	...
	@include(Mpath."data/wapkeyword.php");
	//把图片描述去掉
	//$content=preg_replace("/ alt=([^ >]+)/is","",$content);
	foreach( $Key_word AS $key=>$value){
		if(!$value){
			$value="$webdb[www_url]/news/wapsearch.php?type=title&action=search&keyword=".urlencode($key);
		}
