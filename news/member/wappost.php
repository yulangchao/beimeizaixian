...
if($action=="postnew"){	//新发表内容
	...
	set_user_log(4);	//用户访问日志
 	...
 	refreshto("waplist.php?job=list","<a href='$Mdomain/wapbencandy.php?fid=$fid&id=$id' target='_blank'>查看效果</a> <a href='post.php?fid=$fid'>继续新发表</a> <a href='post.php?job=postmore&fid=$fid&id=$id'>续发第二页</a> <a href='waplist.php?job=list'>返回列表</a>",300);
	...
	$db->query("UPDATE `{$_pre}content` SET pages='$rsdb[pages]' WHERE id='$id'");
 	...
 	refreshto("waplist.php?job=list","<a href='$Mdomain/wapbencandy.php?fid=$fid&id=$id&page=$rsdb[pages]' target='_blank'>查看效果</a> <a href='post.php?job=postmore&fid=$fid&id=$id'>还要续发</a> <a href='waplist.php?job=list'>返回列表</a>",300);
	...
	require(dirname(__FILE__)."/template/wapchoosedel.htm");
	...
	if($rsdb[pages]<2){
		echo "<META HTTP-EQUIV=REFRESH CONTENT='0;URL=wappost.php?job=edit&fid=$rsdb[fid]&id=$rsdb[id]'>";
		exit;
	}
	$query =$db->query("SELECT * FROM `{$_pre}content_1` A WHERE A.id='$id' ORDER BY orderid ASC,rid ASC");
	while($rs =$db->fetch_array($query)){
	...
	require(ROOT_PATH."member/waphead.php");
	require(dirname(__FILE__)."/template/wapchooseedit.htm");
	require(ROOT_PATH."member/wapfoot.php");
	...
	require(ROOT_PATH."member/waphead.php");
	require(dirname(__FILE__)."/template/wappostmore.htm");
	require(ROOT_PATH."member/wapfoot.php");
	...
	refreshto("waplist.php?job=list","<a href='$Mdomain/wapbencandy.php?fid=$fid&id=$id&rid=$rid' target='_blank'>查看效果</a> <a href='waplist.php?job=list'>返回列表</a> <a href='$FROMURL'>继续修改</a>",600);
