...
if($action=='mod'){
	...
	if(strstr($postdb[tplpart_2code],'$picurl')&&!strstr($postdb[tplpart_1code],'$picurl'))
	{
		if(strstr($postdb[tplpart_2code],'$content')){
			$SQL2=" SELECT A.*,R.content FROM {$_pre}content A LEFT JOIN {$_pre}content_1 R ON A.id=R.id $SQL AND A.ispic=1 AND R.topic=1  ORDER BY $order $asc LIMIT $sqlmin,1 ";
		}else{
	...
	if( strstr($postdb[tplpart_1code],'$content')||strstr($postdb[tplpart_2code],'$content') ){
		$SQL=" SELECT A.*,R.content FROM {$_pre}content A LEFT JOIN {$_pre}content_1 R ON A.id=R.id $SQL  AND R.topic=1 $SQLPIC ORDER BY $order $asc LIMIT $sqlmin,$rows ";
	}else{
