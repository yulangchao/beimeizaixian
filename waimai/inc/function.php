...
function fid_cache(){
	...
	write_file(Mpath."data{$webdb[web_dir]}/all_fid.php","<?php\r\n\$Fid_db=".var_export($Fid_db,true).';?>');
	write_file(Mpath."data{$webdb[web_dir]}/guide_fid.php","<?php\r\n\$GuideFid=".var_export($GuideFid,true).';?>');
}
...
function post_info_photo(){
	...
	foreach( $_FILES AS $key=>$value ){
		...
		if($ftype[$i]=='in'&&$array[name]){
			...
			gdpic(ROOT_PATH."$webdb[updir]/{$photodb[$i]}",$Newpicpath,600,450);
...
//删除商家信息
function del_company_info($id,$rs){
	global $db,$_pre;
...
function paymoney_send_seller_msg($uid,$array){
	...
	if($webdb[pay_send_wx2Seller] && $rs[weixin_api]){
		send_wx_msg($rs[weixin_api],"有客户下单付款了,订单号是:$array[id]，请尽快<A HREF='$Murl/member/wapjoinshow.php?id=$array[id]'>查看</A>");
	}
}
...
function select_where($table,$name='fup',$ck='',$fup=''){
	...
	*/
	if(!$fup){
		...
		if(strstr($name,'zone')&&is_file(ROOT_PATH."data{$webdb[web_dir]}/zone/$fup.php")){
			include(ROOT_PATH."data{$webdb[web_dir]}/zone/$fup.php");
