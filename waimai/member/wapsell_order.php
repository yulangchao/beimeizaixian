...
while($rs = $db->fetch_array($query)){
	...
	if($rs[ifpay]){
		...
		if($webdb['daili_receive']){
			...
			$rs[ActOrder].="<A HREF='?job=pay&id=$rs[id]&ifpay=1'>已付款</A> ";
		}
		$rs[ordershow].='未付款 ';
	}
	if($rs[receive]=='-1'){
		$rs[send]="<A style='color:blue;' HREF='?job=send&id=$rs[id]&ifsend=1' title='当前状态未发货,点击确认重新发货'>退货中</A>";
		$rs[ActOrder].="<A HREF='?job=send&id=$rs[id]&ifsend=1'>退货中</A> ";
		$rs[ordershow].='退货中 ';
	}elseif($rs[ifsend]==1){
		if($rs[receive]==1){
		...
		$rs[ActOrder].="<A HREF='?job=send&id=$rs[id]&ifsend=1'>已发货</A> ";
		$rs[ordershow].='未发货 ';
	}
