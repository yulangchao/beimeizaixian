...
if($job=='set'){
	...
	if(!$rsdb){
		echo "<META HTTP-EQUIV=REFRESH CONTENT='0;URL=wappost_company.php'>";
		exit;
	}else{
...
elseif($job=="edit")
{
	...
	require(getTpl("wappost_company"));
...
elseif($action=="edit")
{
	...
	/*更新主信息表内容*/
	$db->query("UPDATE `{$_pre}company` SET fid='$postdb[fid]',title='$postdb[title]',keywords='$postdb[keywords]',picurl='$postdb[picurl]',ispic='$postdb[ispic]',picnum='$num',price='$postdb[price]',city_id='$postdb[city_id]',zone_id='$postdb[zone_id]',street_id='$postdb[street_id]',yz='$rsdb[yz]',sendprice='$postdb[sendprice]',arrive_time='$postdb[arrive_time]',address='$postdb[address]',maps='$postdb[maps]',content='$postdb[content]',telphoto='$postdb[telphoto]' WHERE id='$id'");
