...
$data = array();
//die("SELECT B.*,A.*,C.title AS companyname FROM {$_pre}content A LEFT JOIN {$_pre}content_1 B ON A.id=B.id LEFT JOIN {$pre}hy_company C ON A.uid=C.uid WHERE A.yz='1' $SQL3 $SQL ORDER BY $order_by DESC LIMIT $min,$rows");
$query = $db->query("SELECT B.*,A.*,C.title AS companyname FROM {$_pre}content A LEFT JOIN {$_pre}content_1 B ON A.id=B.id LEFT JOIN {$pre}hy_company C ON A.uid=C.uid WHERE A.yz='1' $SQL3 $SQL ORDER BY $order_by DESC LIMIT $min,$rows");
