...
if($action=="postnew")
{
	...
	/*往标题表插入内容*/
	$db->query("INSERT INTO `{$_pre}content` (`title` , `mid` ,`fid` , `fname` ,  `posttime` , `list` , `uid` , `username` ,  `picurl` , `ispic` , `yz` , `keywords` , `ip` , `money` , `city_id`,`picnum`,`price`,`myfid`) 
	VALUES (
	'$postdb[title]','$fidDB[mid]','$fid','$fidDB[name]','$timestamp','$postdb[list]','$lfjdb[uid]','$lfjdb[username]','$postdb[picurl]','$postdb[ispic]','$postdb[yz]','$postdb[keywords]','$onlineip','$postdb[money]','$postdb[city_id]','$num','$postdb[price]','$postdb[myfid]')");
	...
	refreshto("waplist.php?job=list","<a href='waplist.php?job=list'>返回列表</a> <a target='_blank' href='../wapbencandy.php?city_id=$city_id&fid=$fid&id=$id'>查看效果</a> <a href='post.php?fid=$fid'>继续发布</a>",600);
...
elseif($job=="edit")
{
	...
	$select_mysort = select_mysort($lfjuid,'postdb[myfid]',$rsdb[myfid]);
...
elseif($action=="edit")
{
	...
	/*更新主信息表内容*/
	$db->query("UPDATE `{$_pre}content` SET title='$postdb[title]',keywords='$postdb[keywords]',picurl='$postdb[picurl]',ispic='$postdb[ispic]',picnum='$num',price='$postdb[price]',city_id='$postdb[city_id]',yz='$rsdb[yz]',money='$postdb[money]',myfid='$postdb[myfid]' WHERE id='$id'");
	...
	refreshto("waplist.php?job=list","<a href='waplist.php?job=list'>返回列表</a> <a target='_blank' href='../wapbencandy.php?city_id=$rsdb[city_id]&fid=$fid&id=$id'>查看效果</a>",600);
}
else
{
	/*模块设置时,有些字段有默认值*/
