...
function join_init(){
	extract($GLOBALS);
	global $cid,$p_totalArray,$_ar,$ar,$listdb,$db,$pre,$_pre,$lfjuid;
	...
	if($type=='setcookie'){
		if(!$ts=$db->get_one("SELECT * FROM `{$_pre}car` WHERE uid='$lfjuid' AND cid='$cid'")){
			$num>0 || $num=1;
			$db->query("INSERT INTO `{$_pre}car` ( `cid` , `joins` , `uid` , `type` ) VALUES ( '$cid', '$num', '$lfjuid', '1')");
		}
	}


	$query = $db->query("SELECT * FROM `{$_pre}car` WHERE uid='$lfjuid' ORDER BY id DESC");
	while($rs = $db->fetch_array($query)){
		$p_id = $rs[cid];
		$p_num = $rs[joins];
		//if($p_id<1){
		//	$db->query("DELETE FROM `{$_pre}car` WHERE uid='$lfjuid' AND id='$rs[id]'");
		//	continue;
		//}

		if($p_id==$cid){
			if($type=='setcookie'){	//购物车处理
				if($num<1){
					$db->query("DELETE FROM `{$_pre}car` WHERE uid='$lfjuid' AND id='$rs[id]'");
					continue;
				}elseif($num!=$p_num){
					$db->query("UPDATE {$_pre}car SET joins='$num' WHERE uid='$lfjuid' AND id='$rs[id]'");
				}
				$p_num=$num;			
			}else{
				//$have=true;
	...
	if($cid && !$have){	//新添加的商品  购物车处理 添加新商品
