...
function delete_module($id){
	...
	@unlink($this->tpl_index_new."wappost_$id.htm");
	@unlink($this->tpl_index_new."wapsearch_$id.htm");
	@unlink($this->tpl_index_new."wapbencandy_$id.htm");
	@unlink($this->tpl_index_new."waplist_$id.htm");
}
...
function make_template($id){
	...
	//$wap_post_tpl = $tplDB->read_tpl("wap_post_{$_type}.htm");	//旧版3G发布页
	
	//下面是手机版用到的
	$wapindexshow_tpl = $tplDB->read_tpl("wapbencandy_{$_type}.htm",'wap'.$tpldb['show'],'index');
	$wapindexpost_tpl = $tplDB->read_tpl("wappost_{$_type}.htm",'wap'.$tpldb['post'],'index');
	$wapindexsearch_tpl = $tplDB->read_tpl("wapsearch_{$_type}.htm",'wap'.$tpldb['search'],'index');	
	$wapindexlist_tpl = $tplDB->read_tpl("waplist_{$_type}.htm",'wap'.$tpldb['list'],'index');
	...
	foreach($field_db AS $key=>$rs){
		...
		$tplDB->show_tpl($rs,$wapindexshow_tpl);	//手机版前台内容页

		$tplDB->post_tpl($rs,$indexpost_tpl);	//前台发布页
	
		$rs[listfilter] && $tplDB->listfilter_tpl($rs,$field_db,$indexlist_tpl);	//前台列表页筛选字段

		$rs[listfilter] && $tplDB->listfilter_tpl($rs,$field_db,$wapindexlist_tpl);	//手机列表页筛选字段

		if($rs[listshow]){
			$tplDB->list_tpl($rs,$indexlist_tpl);	//前台列表页
			$tplDB->list_tpl($rs,$indexsearch_tpl);	//前台搜索页

			$tplDB->list_tpl($rs,$wapindexlist_tpl);	//手机前台列表页
			$tplDB->list_tpl($rs,$wapindexsearch_tpl);	//手机前台搜索页
		}
		if($rs[search]){
			$tplDB->search_tpl($rs,$indexsearch_tpl);	//前台搜索页
			$tplDB->search_tpl($rs,$indexlist_tpl);		//前台列表页

			$tplDB->search_tpl($rs,$wapindexsearch_tpl);	//手机前台搜索页
			$tplDB->search_tpl($rs,$wapindexlist_tpl);		//手机前台列表页
		}


		//手机版特别做处理
		if($rs[form_type]=='ieedit'||$rs[form_type]=='ieeditsimp'){
			$rs[form_type]='wapedit';
		}
		$tplDB->post_tpl($rs,$wapindexpost_tpl);	//手机版发布页
		//$tplDB->post_tpl($rs,$wap_post_tpl);	//旧版3G发布页
	}
	...
	//$tplDB->write_tpl("wap_post_$id",$wap_post_tpl,'index');	//旧版3G发布页

	$tplDB->write_tpl("wappost_$id",$wapindexpost_tpl,'index');
	$tplDB->write_tpl("wapsearch_$id",$wapindexsearch_tpl,'index');
	$tplDB->write_tpl("wapbencandy_$id",$wapindexshow_tpl,'index');
	$tplDB->write_tpl("waplist_$id",$wapindexlist_tpl,'index');
}
