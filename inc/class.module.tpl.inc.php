...
function post_tpl($rs,&$tplcode){
	...
	if($rs[form_type]=='text')
	{
		...
		$show0="{$rs[title]}:$mustfill<span>{$rs[form_title]}</span>";
	...
	//WAP可视化编辑器
	elseif($rs[form_type]=='wapedit')
	{
		//$rs[field_inputwidth]>230 || $rs[field_inputwidth]=230;
		//$rs[field_inputheight]>200 || $rs[field_inputheight]=200;
		$show0="{$rs[title]}:$mustfill<span>{$rs[form_title]}</span>";
		$show="<script type='text/javascript'>loadBaiduEditorJs();</script>
			<textarea name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}'>\$rsdb[{$rs[field_name]}]</textarea>$rs[form_js] 
			<script type='text/javascript'>				
			var editor_{$rs[field_name]} = new baidu.editor.ui.Editor({toolbars:[['fullscreen','source','undo','redo','bold', 'italic', 'underline']] ,minFrameHeight:200});
...
\$job!='edit' && \$num=0;\r\nfor( \$i=0; \$i<\$num ;\$i++ ){print <<<EOT\r\n-->
 ...
 地址: 	
                    ...
                    <br></span><!--\r\nEOT;\r\n}print <<<EOT\r\n-->
<div id=\"input_{$rs[field_name]}\$num\"></div>	
...
</SCRIPT><input type='button' onclick='showinput_{$rs[field_name]}()' name='button' value='增加一项'>";
