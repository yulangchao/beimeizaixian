...
if($job=="list")
{
	...
	while($rs = $db->fetch_array($query)){
		...
		$rs[ifyz] = $rs[yz]?"<A HREF='$admin_path&action=work&jobs=unyz&id=$rs[id]' style='color:red;'>已审</A>":"<A HREF='$admin_path&action=work&jobs=yz&id=$rs[id]'>未审</A>";
		$listdb[]=$rs;
	}
...
elseif($action=="work"){
	if($jobs=='com'){
		...
		$db->query("UPDATE {$_pre}person SET yz='1' WHERE id='$id' ");
	}elseif($jobs=="unyz"){
		$db->query("UPDATE {$_pre}person SET yz='0' WHERE id='$id' ");
	}
	refreshto($FROMURL,"操作成功",0);
}
