...
{	
	...
	//手机列表页模板
	if(is_file(Adminpath."template/form_module/tpl/waplist_$id.htm")){
		$waplist_tpl_file=Adminpath."template/form_module/tpl/waplist_$id.htm";
	}else{
		$waplist_tpl_file=Adminpath."template/form_module/tpl/waplist.htm";
	}
	$waplist_tpl=str_replace(array('$TempLate1','$TempLate2',"<",">"),array($tpl_L1,$tpl_L2,"&lt;","&gt;"),read_file($waplist_tpl_file));
	...
	//手机搜索页模板
	if(is_file(Adminpath."template/form_module/tpl/wappost_$id.htm")){
		$wapsearch_tpl_file=Adminpath."template/form_module/tpl/wapsearch_$id.htm";
	}else{
		$wapsearch_tpl_file=Adminpath."template/form_module/tpl/wapsearch.htm";
	}
	$wapsearch_tpl=str_replace(array('$TempLate1','$TempLate2',"<",">"),array($tpl_sarch1,$tpl_sarch2,"&lt;","&gt;"),read_file($wapsearch_tpl_file));
	...
	//手机发布页模板 
	if(is_file(Adminpath."template/form_module/tpl/wappost_$id.htm")){
		$wappost_tpl_file=Adminpath."template/form_module/tpl/wappost_$id.htm";
	}else{
		$wappost_tpl_file=Adminpath."template/form_module/tpl/wappost.htm";
	}
	$wappost_tpl=str_replace(array('$TempLate',"<",">"),array($tpl_p,"&lt;","&gt;"),read_file($wappost_tpl_file));
	...
	//手机内容页模板
	if(is_file(Adminpath."template/form_module/tpl/wapbencandy_$id.htm")){
		$wapshow_tpl_file=Adminpath."template/form_module/tpl/wapbencandy_$id.htm";
	}else{
		$wapshow_tpl_file=Adminpath."template/form_module/tpl/wapbencandy.htm";
	}		
	$wapshow_tpl=str_replace(array('$TempLate',"<",">"),array($tpl_s,"&lt;","&gt;"),read_file($wapshow_tpl_file));
...
{	
	...
	$waptpl_post=stripslashes($waptpl_post);
	$waptpl_list=stripslashes($waptpl_list);
	$waptpl_show=stripslashes($waptpl_show);

	$waptpl_search=stripslashes($waptpl_search);
	...
	write_file(ROOT_PATH."$dirname/data/form_tpl/wappost_$id.htm",$waptpl_post);
	write_file(ROOT_PATH."$dirname/data/form_tpl/wapsearch_$id.htm",$waptpl_search);
	write_file(ROOT_PATH."$dirname/data/form_tpl/wapbencandy_$id.htm",$waptpl_show);
	write_file(ROOT_PATH."$dirname/data/form_tpl/waplist_$id.htm",$waptpl_list);
	...
	if(!is_writable(ROOT_PATH."$dirname/data/form_tpl/wappost_$id.htm")){	
		showmsg("/$dirname/data/form_tpl/wappost_$id.htm模板生成失败,有可能是目录权限不可写,请手工创建一个,复制代码进去");	
	}
	if(!is_writable(ROOT_PATH."$dirname/data/form_tpl/wapsearch_$id.htm")){	
		showmsg("/$dirname/data/form_tpl/wapsearch_$id.htm模板生成失败,有可能是目录权限不可写,请手工创建一个,复制代码进去");	
	}
	if(!is_writable(ROOT_PATH."$dirname/data/form_tpl/wapbencandy_$id.htm")){	
		showmsg("/$dirname/data/form_tpl/wapbencandy_$id.htm模板生成失败,有可能是目录权限不可写,请手工创建一个,复制代码进去");	
	}
	if(!is_writable(ROOT_PATH."$dirname/data/form_tpl/waplist_$id.htm")){	
		showmsg("/$dirname/data/form_tpl/waplist_$id.htm模板生成失败,有可能是目录权限不可写,请手工创建一个,复制代码进去");	
	}
...
{	
	...
	unlink(ROOT_PATH."$dirname/data/form_tpl/list_$id.htm");
	unlink(ROOT_PATH."$dirname/data/form_tpl/post_$id.htm");	
	unlink(ROOT_PATH."$dirname/data/form_tpl/bencandy_$id.htm");	
	unlink(ROOT_PATH."$dirname/data/form_tpl/search_$id.htm");
	
	unlink(ROOT_PATH."$dirname/data/form_tpl/waplist_$id.htm");	
	unlink(ROOT_PATH."$dirname/data/form_tpl/wappost_$id.htm");
	unlink(ROOT_PATH."$dirname/data/form_tpl/wapbencandy_$id.htm");
	unlink(ROOT_PATH."$dirname/data/form_tpl/wapsearch_$id.htm");

	unlink(ROOT_PATH."$dirname/data/form_tpl/admin_bencandy_$id.htm");
	unlink(ROOT_PATH."$dirname/data/form_tpl/admin_list_$id.htm");
...
function make_post_table($rs){	
	...
	{	
		if($rs[field_inputleng]){
			if($rs[field_inputleng]<100){
				$thiswidth="style='width:100px;'";
			}else{
				$thiswidth="style='width:{$rs[field_inputleng]}px;'";
			}
		}else{
			$thiswidth="";
		}
		$show="<dl>\r\n<dt>$mustfill {$rs[title]}:{$rs[form_title]}</dt>\r\n<dd><input type='text' name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' $thiswidth value='\$rsdb[{$rs[field_name]}]'> $rs[form_units]</dd>\r\n</dl>\r\n";	
	}
	elseif($rs[form_type]=='time')	
	{	
		$show="<dl>\r\n<dt>$mustfill {$rs[title]}:{$rs[form_title]}</dt>\r\n<dd><input onclick=\"setday(this,1)\" type='text' name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' style='width:100px;' value='\$rsdb[{$rs[field_name]}]'>$rs[form_units]</dd>\r\n</dl>\r\n";	
	}
	elseif($rs[form_type]=='upfile')	
	{	
		$show="<dl>\r\n<dt>$mustfill {$rs[title]}:{$rs[form_title]}</dt>\r\n<dd><input type='text' name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' style='width:300px;' value='\$rsdb[{$rs[field_name]}]'> $rs[form_units] <iframe frameborder=0 height=23 scrolling=no src='\$webdb[www_url]/do/upfile.php?fn=upfile&dir=\$_pre\$fid&label=atc_{$rs[field_name]}' width=310></iframe></dd>\r\n</dl>\r\n";	
	}	
	elseif($rs[form_type]=='upmorefile')	
	{	
		$show="<dl>\r\n<dt>$mustfill {$rs[title]}:{$rs[form_title]}<div>增加 <input type='text' style='width:30px;' name='nums_{$rs[field_name]}' value='2'> 项 <input type='button' name='Submit2' value='增加' onClick='showinput_{$rs[field_name]}()'></div></dt>\r\n<dd><!--	
EOT;
...
--> 名称: <input type=\"text\" name=\"postdb[{$rs[field_name]}][name][]\" id=\"atc_{$rs[field_name]}_name\$i\" size=\"15\" value=\"{\$rsdb[{$rs[field_name]}][name][\$i]}\">	
 ...
 地址: 	
                    <input type=\"text\" name=\"postdb[{$rs[field_name]}][url][]\" id=\"atc_{$rs[field_name]}_url\$i\" style='width:300px;' value=\"{\$rsdb[{$rs[field_name]}][url][\$i]}\">	
...
function upfile_{$rs[field_name]}(url,name,size,label){	
	...
	{	
		$show="<dl>\r\n<dt>$mustfill {$rs[title]}:{$rs[form_title]}</dt>\r\n<dd><textarea name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}'>\$rsdb[{$rs[field_name]}]</textarea>$rs[form_units]</dd>\r\n</dl>\r\n";	
	}	
	elseif($rs[form_type]=='ieedit')	
	{	
		$show="<dl>\r\n<dt>$mustfill {$rs[title]}:{$rs[form_title]}</dt>\r\n<dd><iframe id='eWebEditor1' src='\$webdb[www_url]/ewebeditor/ewebeditor.php?id=atc_{$rs[field_name]}&style=standard&&etype=1' frameborder='0' scrolling='no' width='90%' height='250'></iframe>$rs[form_units]<input name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}' type='hidden' value='\$rsdb[{$rs[field_name]}]'></dd>\r\n</dl>\r\n";	
	}	
	...
	{	
		...
		$show="<dl>\r\n<dt>$mustfill {$rs[title]}:{$rs[form_title]}</dt>\r\n<dd><select name='postdb[{$rs[field_name]}]' id='atc_{$rs[field_name]}'>$_show</select>$rs[form_units]</dd>\r\n</dl>\r\n";	
	}	
...
function make_show_table($rs){	
	...
	$show="<dl>\r\n<dt>{$rs[title]}:</dt>\r\n<dd>{\$rsdb[{$rs[field_name]}]} {$rs[form_units]}</dd>\r\n</dl>\r\n";	
	if($rs[form_type]=='pingfen'){	
