...
if($job=='read')
{
	...
	require(dirname(__FILE__)."/"."template/pm/wapread.htm");
...
elseif($job=='send')
{
	...
	require(dirname(__FILE__)."/"."waphead.php");
	require(dirname(__FILE__)."/"."template/pm/wapsend.htm");
	require(dirname(__FILE__)."/"."wapfoot.php");
}else{
	if(!$page){
		$page=1;
	...
	while($rs = $db->fetch_array($query))
	{
		...
		$rs[mdate]=date("Y-m-d",$rs[mdate]);
		$listdb[]=$rs;
	}
	require(dirname(__FILE__)."/"."waphead.php");
	require(dirname(__FILE__)."/"."template/pm/waplist.htm");
	require(dirname(__FILE__)."/"."wapfoot.php");	
}
