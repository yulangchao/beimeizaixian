...
while($rs = $db->fetch_array($query)){
	...
	$rs[url1] =  "$webdb[www_url]/tg/member/wapjoin.php?fid=$rs[fid]&cid=$rs[id]";
	$rs[totaluser] = $rs[totaluser]?"已有$rs[totaluser]人参团":"欢迎报名参加";
...
if($job=="showmore"){
	if(!$listdb){
		...
		<dd class='R'>
		  ...
		  <p class='ListPrice'><span class='L'>$rs[totaluser]</span><span class='R'><a href='$rs[url1]'>立即报名</a></span></p>
		  <p class='ListOther'><span class='L'>团长：$rs[username]</span><span class='R'>申请时间：$rs[posttime]</span></p>
		</dd>
	  </dl>";
		}
	}
