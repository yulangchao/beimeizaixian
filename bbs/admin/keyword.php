...
elseif($action=="getword" && ck_power('keyword'))
{
	...
	if($iii==0&&$page<2)
	{
		...
		//write_file("../cache{$webdb[web_dir]}/makeShow0.php",$allfid);
	}
	else
	{
		//$allfid=read_file("../cache{$webdb[web_dir]}/makeShow0.php");
...
function make_keyword_cache(){
	...
	write_file(Mpath."data{$webdb[web_dir]}/keyword.php","$show ?>");
}
