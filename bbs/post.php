...
if($action=="postnew"){	//新发表内容
	...
	if( !$postdb[picurl] && $file_db=get_content_attachment($postdb[content]) ){
		if( $file_db[0] && eregi("(jpg|gif|png)$",$file_db[0]) && !eregi("sysimage\/file",$file_db[0]) ){
			...
			if($webdb[if_gdimg]){			
				...
				$Newpicpath=ROOT_PATH."$webdb[updir]/$postdb[picurl]";
				gdpic(ROOT_PATH."$webdb[updir]/$file_db[0]",$Newpicpath,600,450);
	...
	set_user_log(4);	//用户访问日志
 	...
 	refreshto("$Mdomain/bencandy.php?fid=$fid&id=$id","<a href='$Mdomain/bencandy.php?fid=$fid&id=$id' target='_blank'>查看效果</a> <a href='post.php?fid=$fid'>继续新发表</a> <a href='post.php?job=postmore&fid=$fid&id=$id'>续发第二页</a> <a href='list.php?job=list'>返回列表</a>",0);
	...
	require(dirname(__FILE__)."/template/default/post.htm");
	...
	if( !$postdb[picurl] && $file_db=get_content_attachment($postdb[content]) ){
		if( $file_db[0] && eregi("(jpg|gif)$",$file_db[0]) && !eregi("sysimage\/file",$file_db[0]) ){
			...
			if($webdb[if_gdimg])
			{
				$postdb[picurl]=str_replace(".","_",$file_db[0]).'.jpg';
				$Newpicpath=ROOT_PATH."$webdb[updir]/$postdb[picurl]";
				gdpic(ROOT_PATH."$webdb[updir]/$file_db[0]",$Newpicpath,600,450);
				if(!file_exists($Newpicpath)){
					$postdb[picurl]=$file_db[0];
