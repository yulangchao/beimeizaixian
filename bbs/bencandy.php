...
if($IsMob){
	header("location:wapbencandy.php?fid=$fid&id=$id&introducer_uid=$introducer_uid");
	exit;
}
...
$min=($page-1)*$rows;
$sowComments="";
$query = $db->query("SELECT SQL_CALC_FOUND_ROWS * FROM {$_pre}comments WHERE id='$id' AND cids='0' ORDER BY cid DESC LIMIT $min,$rows");
$RS=$db->get_one("SELECT FOUND_ROWS()");
$totalNum=$RS['FOUND_ROWS()'];
$showpage = getpage("","","?fid=$fid&id=$id",$rows,$totalNum);
$showpage=preg_replace("/\?fid=([\d]+)&id=([\d]+)&page=([\d]+)/is","javascript:showpagecomments(\\1,\\2,\\3)",$showpage);
while($rs = $db->fetch_array($query)){
	...
	$havecomments=($listrepeatcomment=="<div class='ShowMoreRepaly'></div>\r\n")?'':'havecomments';
	$memberinfo=get_members_baseInfo($rs[uid]);
	$delword=($rs[uid]==$lfjuid)?"<span class='delcomment' onClick='del_comments($id,$rs[cid],$rs[uid])'><em>删除</em></span>":"";
	$show="";
	$show.="<table class='InfoTable'>\r\n";
	$show.=" <tr>\r\n";
	$show.="  <td class='member'>\r\n";
	$show.="    <div class='icon'><A HREF='$webdb[www_url]/member/userinfo.php?uid=$memberinfo[uid]'><img src='$memberinfo[icon]' onerror=\"this.src='$webdb[www_url]/images/default/nobody.gif'\"/></A></div>\r\n";
	$show.="    <div class='username'><A HREF='$webdb[www_url]/member/userinfo.php?uid=$memberinfo[uid]'>$memberinfo[username]</A></div>\r\n";
	$show.="    <div class='grouptitle'>{$memberinfo[grouptitle]}</div>\r\n";
	$show.="    <div class='time'>注册：{$memberinfo[regdate]}</div>\r\n";
	$show.="    <div class='time'>登录：{$memberinfo[lastvist]}</div>\r\n";
	$show.="  </td>\r\n";
	$show.="  <td class='infos'>\r\n";
	$show.="    <div class='content'>{$rs[content]}</div>\r\n";
	$show.="    <div class='posttime'><span>$rs[posttime]</span> <span class='repeat' onClick='add_comments($id,$rs[cid])'><em>回复</em></span> {$delword}</div>\r\n";
	$show.="	<div class='Listrepeatcomment Listrepeatcomment$rs[cid] $havecomments' title='$rs[cid]'>\r\n{$listrepeatcomment}\r\n</div>\r\n";
	$show.="  </td>\r\n";
	$show.=" </tr>\r\n";
	$show.="</table>\r\n";
	$sowComments.=$show;
...
if($showpage){
	$sowComments.="<div class='ShowPage'>$showpage</div>";
}
...
if($atc=='comment'){//回帖操作
	...
	//$content=filtrate($_POST[content]);
	...
	//图片目录转移
	$content=move_attachment($lfjuid,$content,"{$_pre}/$fid");
	...
	header("location:bencandy.php?fid=$fid&id=$id");
	exit;
}
