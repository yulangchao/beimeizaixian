...
function label_hf(){
	...
	while($rs = $db->fetch_array($query)){
		if( $rs[type]=='code' ){
			...
			if(is_array($_detail)){
				$labelArrayS.="\$HFlabelArray=@unserialize(stripslashes('".addslashes($rs[code])."'));";
				$labelArrayS.="
							if(isset(\$HFlabelArray[\$city_id])){
								\$label['{$rs[tag]}']=En_TruePath(\$HFlabelArray[\$city_id],0);
							}else{
								\$label['{$rs[tag]}']=En_TruePath(\$HFlabelArray[0],0);
							}
							";
			}else{
				$label[$rs[tag]]=stripslashes( En_TruePath($rs[code],0) );
			}

		}elseif( $rs[type]=='pic' ){
			//多城市新加功能
			$_detail=unserialize($rs[code]);
			if(is_array($_detail[0])){
				$labelArrayS.="\$HFlabelArray=@unserialize(stripslashes('".addslashes($rs[code])."'));";
				$labelArrayS.="
							...
							if(\$labelPicDb['imglink']){
								\$label['{$rs[tag]}']=\"<a href='\$labelPicDb[imglink]' target='_blank'><img src='\$labelPicDb[imgurl]' width='\$labelPicDb[width]' height='\$labelPicDb[height]' border='0' /></a>\";
							}else{
								\$label['{$rs[tag]}']=\"<img src='\$labelPicDb[imgurl]' width='\$labelPicDb[width]' height='\$labelPicDb[height]' border='0' />\";
							}							
							";
			}else{
				...
				if($picdb['imglink']){
					$label[$rs[tag]]="<a href='$picdb[imglink]' target=_blank><img src='$picdb[imgurl]' width='$picdb[width]' height='$picdb[height]' border='0' /></a>";
				}else{
					$label[$rs[tag]]="<img src='$picdb[imgurl]' width='$picdb[width]' height='$picdb[height]'  border='0' />";
				}
			}
		}elseif( $rs[type]=='swf' ){
			...
			$label[$rs[tag]]="<object type='application/x-shockwave-flash' data='$flashdb[flashurl]' $width $height wmode='transparent'><param name='movie' value='$flashdb[flashurl]' /><param name='wmode' value='transparent' /></object>";
		}else{
			$label[$rs[tag]]=stripslashes($rs[code]);
		}		
	}
