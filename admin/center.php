...
elseif($job=='weixin_api'&&$Apower[weixin_api])
{
 
	require(dirname(__FILE__)."/"."head.php");
	require(dirname(__FILE__)."/"."template/center/menu.htm");
	require(dirname(__FILE__)."/"."template/center/weixin_api.htm");
	require(dirname(__FILE__)."/"."foot.php");
}
...
elseif($job=="adminlogin"&&$Apower[center_config])
{
	require(dirname(__FILE__)."/"."head.php");
	require(dirname(__FILE__)."/"."template/center/menu.htm");
	require(dirname(__FILE__)."/"."template/center/adminlogin.htm");
	require(dirname(__FILE__)."/"."foot.php");
}
...
elseif($action=='cache'&&($Apower[center_cache]||$Apower[center_config]))
{
	del_file(ROOT_PATH."cache/index_cache/");
	write_config_cache($webdbs);
	jump("设置成功",$FROMURL);
}
...
/* 信息统计 */
	...
	$tagpicurl = "$webdb[www_url]/member/images/wap";
	$tagpic = array("20"=>"<img src='{$tagpicurl}/icon3.png'>","38"=>"<img src='{$tagpicurl}/icon4.png'>","39"=>"<img src='{$tagpicurl}/icon5.png'>","50"=>"<img src='{$tagpicurl}/icon6.png'>","36"=>"<img src='{$tagpicurl}/icon7.png'>","29"=>"<img src='{$tagpicurl}/icon8.png'>","55"=>"<img src='{$tagpicurl}/icon9.png'>","51"=>"<img src='{$tagpicurl}/icon10.png'>","16"=>"<img src='{$tagpicurl}/icon11.png'>","28"=>"<img src='{$tagpicurl}/icon12.png'>","37"=>"<img src='{$tagpicurl}/icon13.png'>","31"=>"<img src='{$tagpicurl}/icon14.png'>","26"=>"<img src='{$tagpicurl}/icon15.png'>","27"=>"<img src='{$tagpicurl}/icon16.png'>","40"=>"<img src='{$tagpicurl}/icon17.png'>","56"=>"<img src='{$tagpicurl}/icon17.png'>");
	foreach($ModuleDB AS $key=>$mds){
		if(!$mds[ifclose] && $tagpic[$mds[id]]){
			$tagpic[$mds[id]] || $tagpic[$mds[id]] ="<img src='$webdb[www_url]/member/images/wap/icon7.png'/>";
			if($key == "hy_"){
				@extract($db->get_one("SELECT COUNT(*) AS num FROM {$pre}{$key}company WHERE yz='0'"));
				@extract($db->get_one("SELECT COUNT(*) AS nums FROM {$pre}{$key}company"));
				if($num){
					$show.="<li>{$mds[name]}：<span><a href='?lfj=module_admin&dirname={$mds[dirname]}&file=company&job=list&type=unyz'>{$num}</a></span> 条</li>";
				}
				if($nums){
					$show2.="<li>{$tagpic[$mds[id]]}{$mds[name]}：<span><a href='?lfj=module_admin&dirname={$mds[dirname]}&file=company&job=list'>{$nums}</a></span> 条</li>";
				}
				
			}else{
				@extract($db->get_one("SELECT COUNT(*) AS num FROM {$pre}{$key}content WHERE yz='0'"));
				@extract($db->get_one("SELECT COUNT(*) AS nums FROM {$pre}{$key}content"));
				if($num){
					$show.="<li>{$mds[name]}：<span><a href='?lfj=module_admin&dirname={$mds[dirname]}&file=list&job=list&type=unyz'>{$num}</a></span> 条</li>";
				}
				if($nums){
					$show2.="<li>{$tagpic[$mds[id]]}{$mds[name]}：<span><a href='?lfj=module_admin&dirname={$mds[dirname]}&file=list&job=list'>{$nums}</a></span> 条</li>";
				}
	...
	@extract($db->get_one("SELECT COUNT(*) AS Member FROM {$pre}memberdata WHERE yz='0'"));
	@extract($db->get_one("SELECT COUNT(*) AS Members FROM {$pre}memberdata"));
	$show = "<ul>
	    <li>会员共有：<span><a href='?lfj=member&job=list&T=unyz'>{$Member}</a></span> 位</li>".$show."</ul>";
	$show2 = "<ul>
	    <li><img src='$webdb[www_url]/member/images/wap/icon1.png'/>会员共有：<span><a href='?lfj=member&job=list'>{$Members}</a></span> 位</li>".$show2."</ul>";
