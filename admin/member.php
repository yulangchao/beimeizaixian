...
if($job=="list"&&($Apower[member_list]||$Apower[member_qy_list]))
{
	...
	while($rs=$db->fetch_array($query)){
		...
		if(defined('IN_QY')&&$rs[groupid]){
			...
			$rs[editqy] = "<A HREF='$webdb[www_url]/hy/member/post_company.php?uid=$rs[uid]' title='修改企业信息' target='_blank' class='edit'>修改企业信息</A>";
...
elseif($action=="editmember"&&($Apower[member_list]||$Apower[member_qy_list]))
{
	...
	if(($rsdb[idcard_yz]!=1) && ($array[idcard_yz]==1)){
		add_user($uid,$webdb[YZ_IdcardMoney],'通过身份验证奖励积分');
	}
