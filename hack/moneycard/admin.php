...
elseif($action=='make'&&$Apower[moneycard_make])
{
	...
	for($i=0;$i<$atc_num;$i++){
		...
		for($j=0;$j<$atc_leng ;$j++ ){
			if($atc_type==1){
				$startnum=mt_rand(0,9);
			}elseif($atc_type==2){
				$startnum=mt_rand(10,25);
			}else{
				$startnum=mt_rand(0,25);
			}
			$passwd.=substr($letter_str,$startnum,1);
		}
		if( !$db->get_one("SELECT * FROM  `{$pre}moneycard` WHERE `passwd`='$passwd'") ){
...
elseif($job=='list'&&$Apower[moneycard_make])
{
	...
	$SQL='';
	if($atc_passwd){
		$SQL=" WHERE passwd='$atc_passwd' ";
	}
	$query = $db->query("SELECT * FROM `{$pre}moneycard` $SQL ORDER BY id DESC LIMIT $min,$rows");
	$showpage=getpage("{$pre}moneycard $SQL","","?lfj=$lfj&job=$job",$rows);
