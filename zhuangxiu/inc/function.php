...
function List_Company_Demo($id,$rows){
	global $db,$_pre;
	$query = $db->query("SELECT B.*,C.title,C.city_id,C.picurl FROM {$_pre}content_6 A LEFT JOIN {$_pre}content_2 B ON A.id=B.designer_id  LEFT JOIN {$_pre}content C  ON B.id=C.id  WHERE A.company_id='$id' ORDER BY B.id DESC LIMIT $rows");
...
function List_Company_Designer($id,$rows){
	global $db,$_pre;
	$query = $db->query("SELECT A.*,B.title,B.city_id,B.picurl FROM {$_pre}content_6 A LEFT JOIN {$_pre}content B ON A.id=B.id WHERE A.company_id='$id' ORDER BY A.id DESC LIMIT $rows");
	while($rs = $db->fetch_array($query)){
		$listdb[] = $rs;
	}
	return $listdb;
}
function Show_Company_Demo_Num($id){
	global $db,$_pre;
	$RS=$db->get_one("SELECT COUNT( * ) AS NUM FROM {$_pre}content_6 A LEFT JOIN {$_pre}content_2 B ON A.id = B.designer_id WHERE A.company_id =  '$id'");
	return $RS[NUM];
}
function Company_Designer_Num($id){
	global $db,$_pre;
	$RS=$db->get_one("SELECT COUNT(*) AS NUM FROM {$_pre}content_6 WHERE company_id='$id'");
	return $RS['NUM'];
}
function List_Designer_Demo($id,$rows){
	global $db,$_pre;
	$query = $db->query("SELECT A.*,B.title,B.city_id,B.picurl FROM {$_pre}content_2 A LEFT JOIN {$_pre}content B ON A.id=B.id WHERE A.designer_id='$id' ORDER BY A.id DESC LIMIT $rows");
	while($rs = $db->fetch_array($query)){
		if($rs[picurl]){
