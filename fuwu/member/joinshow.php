...
*获取信息正文的内容
**/
$rsdb=$db->get_one("SELECT A.*,B.*,M.icon FROM `{$_pre}join` A LEFT JOIN `{$_pre}content_$mid` B ON A.id=B.rid LEFT JOIN {$pre}memberdata M ON A.uid=M.uid WHERE A.id='$id'");
