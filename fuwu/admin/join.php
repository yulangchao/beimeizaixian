...
if($job=='yz'){
	...
	foreach($iddb AS $key=>$value){
		...
		$db->query("DELETE FROM {$_pre}content_2 WHERE rid='$value'");
	}
}
...
$query = $db->query("SELECT SQL_CALC_FOUND_ROWS B.*,A.* FROM {$_pre}join A LEFT JOIN {$_pre}content_2 B ON A.id=B.rid  ORDER BY A.id DESC LIMIT $min,$rows");
